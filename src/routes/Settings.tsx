...
function Toggle({ on, onClick }: { on?: boolean; onClick?: () => void }) {
  return (
    ...
    >
      <span
        className={
          "absolute top-[2px] h-3.5 w-3.5 rounded-full bg-white shadow transition-[left] " +
...
export default function Settings() {
  ...
  useEffect(() => {
    localStorage.setItem(AUTOCHECK_KEY, String(autoCheck));
  }, [autoCheck]);
    ...
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="grid grid-cols-[172px_1fr] gap-9 overflow-auto px-9 pb-7 pt-5">
        <nav className="flex flex-col gap-px text-[13px]">
          <div className="rounded-md bg-accent/10 px-2.5 py-1.5 font-semibold text-accent">
            常规
          </div>
        ...
        <div className="flex min-w-0 flex-col gap-6">
          <section>
            <div className="mb-2.5 text-[11px] font-semibold uppercase tracking-wider text-ink-50">
              常规
            </div>
            <Card className="overflow-hidden">
              <Row label="语言">
                <span className="rounded-sm border border-rule bg-white px-2 py-0.5 text-[12px]">
                  简体中文
                </span>
              </Row>
              <Row label="启动时打开">
            ...
            <Card className="overflow-hidden">
              <div className="flex items-center gap-3 border-b border-rule bg-accent-soft px-4 py-3">
                <span className="relative inline-flex h-2 w-2 items-center justify-center">
                  <span
                    className={
                      "absolute inset-0 rounded-full " + (s?.connected ? "bg-accent" : "bg-ink-30")
                    }
                  />
                </span>
                <div className="flex-1">
                  <div className="text-[13px] font-semibold">
                    处理引擎 {s?.connected ? "已连接" : "未连接"}
                  </div>
                  <div className="mt-0.5 font-mono text-[11.5px] text-ink-50">
                    pivot-sidecar · {s?.version ?? "–"} · Python {s?.python_version ?? "–"} · pid{" "}
                    {s?.pid ?? "–"}
                  </div>
                </div>
                <Button variant="secondary" onClick={() => ipc.restartSidecar()}>
                  重启
                </Button>
